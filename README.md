...
- Supports opt-in (GDPR/ePrivacy, UK GDPR/PECR, LGPD, Law 25, POPIA, KVKK) and opt-out (CCPA/CPRA and other US state laws) consent models, plus browser and `Sec-GPC` Global Privacy Control signals.
...
- Supports geo-based automatic model switching by visitor country and ISO 3166-2 subdivision.
...
Cookie's **Compact HTML output** option is enabled by default. It removes
comments and formatting whitespace from the module's inline CSS and widget
markup while leaving visitor-facing text and the rest of the page untouched.
Disable it under **Advanced** when readable source is useful for debugging.

### Sharing consent with subdomains

Set **Advanced > Consent cookie domain** to a parent domain such as
`example.com` when the same consent choice must be available on trusted sibling
applications such as `agenda.example.com`. Leave it empty (the default) to keep
the cookie host-only. A leading dot is unnecessary and is normalized away.

All participating applications must use the same category keys and consent
policy version. Non-ProcessWire applications must read the `pwcm_consent` JSON
and apply the corresponding consent state before their GTM/gtag tags run; the
shared cookie does not install Cookie's Google Consent Mode integration on
those applications. Because every subdomain can read and overwrite a
domain-scoped cookie, enable this only when all subdomains share the same trust
boundary.

Enabling the setting is migrated automatically the next time consent is saved:
Cookie removes the old host-only value before writing the parent-domain value.
If you later remove or narrow the setting, explicitly expire the old cookie
with its former `Domain` attribute during deployment (or move to a new consent
cookie name); a host-only write cannot delete a parent-domain cookie.
...
Enable *Geo mode* (module config) to pick the model by visitor country and
region
...
(Cloudflare and many CDNs set it); the default subdivision header is
`CF-Region-Code`, available through Cloudflare's Add visitor location headers
Managed Transform. Rules use ISO 3166-2 values such as `CA-QC`, and subdivision
rules take priority over country rules. Quebec is included as an opt-in region
by default. Override the headers, set `$config->geoCountry` / `$config->geoRegion`,
or hook `Cookie::detectCountry` / `Cookie::detectRegion` for a GeoIP library.
With full-page caching, shared HTML contains a conservative fallback and the
frontend resolves regional and `Sec-GPC` state from a private, session-free,
no-store endpoint before starting the consent manager.

### Global Privacy Control

When *Respect GPC* is enabled, Cookie honors both
`navigator.globalPrivacyControl` and the `Sec-GPC: 1` request header. It keeps
marketing refused, suppresses the normal banner for that visit, and displays a
temporary configurable confirmation. The floating settings control remains
available. The signal is resolved through the same private endpoint as geo
mode so it is not leaked between visitors by a full-page cache.
...
`getIconSvg`, `resolveConsentModel`, `detectCountry`, `detectRegion`, `detectGpc`.
...
Only known config keys are applied (unknown keys are ignored); consent-log
records are never transferred. Programmatic: `$cookie->exportSettings()` /
...
Upgrading from 1.2.x assigns random identifiers to existing log rows and removes
the old `ip_hash` column and obsolete salt setting. Back up the consent-log table
before upgrading if those legacy hashes must be retained for an external audit.
